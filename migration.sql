...
drop policy if exists "Class owners/coaches can view managed class reservations." on public.fencing_class_reservations;
...
drop policy if exists "Class owners/coaches can update managed class reservations." on public.fencing_class_reservations;
...
drop policy if exists "Coaches can view incoming lesson orders." on public.fencing_lesson_orders;
...
drop policy if exists "Coaches can update incoming lesson orders." on public.fencing_lesson_orders;
...
-- LESSON REVIEWS
create table if not exists public.fencing_lesson_reviews (
  id uuid default uuid_generate_v4() primary key,
  lesson_id uuid references public.fencing_lesson_products(id) on delete cascade not null,
...
create unique index if not exists idx_fencing_lesson_reviews_unique
...
drop policy if exists "Lesson reviews are viewable by everyone." on public.fencing_lesson_reviews;
create policy "Lesson reviews are viewable by everyone."
  on public.fencing_lesson_reviews for select
  using ( true );

drop policy if exists "Users can insert lesson reviews for ordered lessons." on public.fencing_lesson_reviews;
...
drop policy if exists "Users can update own lesson reviews." on public.fencing_lesson_reviews;
create policy "Users can update own lesson reviews."
  on public.fencing_lesson_reviews for update
  using ( auth.uid() = reviewer_id );

drop policy if exists "Users can delete own lesson reviews." on public.fencing_lesson_reviews;
create policy "Users can delete own lesson reviews."
  on public.fencing_lesson_reviews for delete
  using ( auth.uid() = reviewer_id );

create index if not exists idx_fencing_lesson_reviews_lesson_id
  on public.fencing_lesson_reviews(lesson_id);
create index if not exists idx_fencing_lesson_reviews_reviewer_id
